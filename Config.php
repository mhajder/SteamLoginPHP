...
// System stuff
if (empty($steamlogin['apikey'])) {
    die("<div style='display: block; width: 100%; color: red; text-align: center;'>STEAMLOGIN:<br>SET YOUR APIKEY IN THE CONFIG.PHP!!</div>");
}
if (empty($steamlogin['domainname'])) {
    $steamlogin['domainname'] = $_SERVER['SERVER_NAME'];
}
if (empty($steamlogin['logoutpage'])) {
    $steamlogin['logoutpage'] = $_SERVER['PHP_SELF'];
}
if (empty($steamlogin['loginpage'])) {
    $steamlogin['loginpage'] = $_SERVER['PHP_SELF'];
}